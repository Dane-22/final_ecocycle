...
// Fetch only Best Seller products from database (products with 5+ successful deliveries)
try {
    $query = "
        SELECT p.*, c.name as category_name, s.fullname as seller_name,
               COUNT(CASE WHEN oi.status = 'delivered' THEN 1 END) as sales_count
        FROM products p 
    ...
    $query .= " GROUP BY p.product_id
        HAVING COUNT(CASE WHEN oi.status = 'delivered' THEN 1 END) >= 5
...
<html lang="en">
  <head>
    <title>Best Seller Products - Ecocycle NLUC</title>
    <meta charset="utf-8">
      ...
      <div class="row">
        <div class="main-content">
          ...
          <div class="container-lg mt-3" id="recyclingProducts">
            <h3 class="fw-bold mb-4 text-warning">Best Seller Products</h3>
            <div class="row">
              <?php if (empty($products)): ?>
                <div class="col-12 text-center">
                  <img src="images/logo.png.png" alt="No Products" style="width: 100px; opacity: 0.5; margin-bottom: 20px;">
                  <p class="lead">No Best Seller products available yet.</p>
                </div>
              <?php else: ?>
