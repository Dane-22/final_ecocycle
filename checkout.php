...
if (isset($_SESSION['user_id']) && isset($_SESSION['user_type']) && $_SESSION['user_type'] === 'buyer') {
    ...
    foreach ($cart_items as $item) {
        if ($item) {
            if ($item['shipping_type'] === 'size_based' && $item['size'] === 'small') {
            ...
            $handling_fee += $category_fee * $item['quantity'];
        }
    }
    ...
    // Calculate shipping for each cart item
    foreach ($cart_items as $item) {
        if ($item) {
            if ($item['shipping_type'] === 'size_based' && $item['size'] === 'small') {
                // Small Box flat rate
    ...
    // Calculate EcoCoins equivalents (Peso price + 20 per unit for items)
                            ...
                            <div class="card shadow-sm border-0 mb-4">
                                ...
                                <div class="card-body">
                                    <form id="checkoutForm" method="POST" action="codpayment.php">
                                        ...
                                        <input type="hidden" name="amount_ecocoins" id="hiddenTotalEco" value="<?php echo htmlspecialchars($total_ecocoins); ?>">
                                        ...
                                        <?php if (!empty($cart_items)): ?>
                                            <?php foreach ($cart_items as $item): ?>
                                                <div class="d-flex justify-content-between mb-1">
                                                    ...
                                                    <span class="cartItemPrice">₱<?php echo number_format($item['price'] * $item['quantity'], 2); ?></span>
                                                    <span class="cartItemPriceEco" style="display:none;"><?php echo number_format($item['price'] * $item['quantity'] + (20 * $item['quantity']), 2); ?> EcoCoins</span>
                                                </div>
                                            <?php endforeach; ?>
                                        <?php else: ?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function togglePaymentDetails() {
            if (gcashRadio && gcashRadio.checked) {
                ...
                document.querySelectorAll('.cartItemPrice').forEach(el => el.style.display = 'inline');
                document.querySelectorAll('.cartItemPriceEco').forEach(el => el.style.display = 'none');
                ...
                document.querySelectorAll('.cartItemPrice').forEach(el => el.style.display = 'none');
                document.querySelectorAll('.cartItemPriceEco').forEach(el => el.style.display = 'inline');
            } else {
                gcashDetails.style.display = 'none';
                if (ecocoinsDetails) ecocoinsDetails.style.display = 'none';
                // Show peso prices
                ...
                fetch('api/order-summary.php')
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            // Also update ecocoins total
                            if (data.total_ecocoins) {
                                document.getElementById('hiddenTotalEco').value = data.total_ecocoins;
                            }
                        }
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                if (paymentMethod === 'cod') {
                                    ...
                                    let totalEcocoins = document.getElementById('hiddenTotalEco').value;
                                    window.location.href = `ecocoins-payment.php?amount=${encodeURIComponent(totalEcocoins)}&order_id=ECO-${Date.now()}`;
